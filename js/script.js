$(function(){
  ...
  $(function(){
	$('.minimized--tel').click(function(event) {
	  var i_path = $(this).attr('src');
	  $('body').append('<div id="overlay"></div><div id="magnify--tel"><img src="'+i_path+'"><div id="close-popup"><i></i></div></div>');
	  $('#magnify--tel').css({
		  left: ($(document).width() - $('#magnify--tel').outerWidth())/2,
		  // top: ($(document).height() - $('#magnify').outerHeight())/2 upd: 24.10.2016
			  top: ($(window).height() - $('#magnify--tel').outerHeight())/2
		});
	  $('#overlay, #magnify--tel').fadeIn('fast');
	});
	
	$('body').on('click', '#close-popup, #overlay', function(event) {
	  event.preventDefault();
   
	  $('#overlay, #magnify--tel').fadeOut('fast', function() {
		$('#close-popup, #magnify--tel, #overlay').remove();
	  });
	});
  });
